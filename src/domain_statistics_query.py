...
OUTPUT_EXCEL_PATH: Path = Path(
    r"D:\ai\work\도메인중복데이터_결과.xlsx"
)
SHEET_NAME: str = "stastics"
...
def get_db_password() -> str:
    """데이터베이스 비밀번호를 환경 변수에서 조회한다."""
    password: Optional[str] = os.getenv("PGPASSWORD") or os.getenv("DB_PASSWORD")
    if not password:
        raise ValueError(
            "PostgreSQL 비밀번호가 필요합니다. "
            "환경 변수 PGPASSWORD 또는 DB_PASSWORD를 설정하세요."
        )
    return password
...
def read_statistics_sheet() -> pd.DataFrame:
    """입력 Excel의 stastics 시트 데이터를 읽는다."""
    ...
    if raw_dataframe.empty:
        raise ValueError("stastics 시트에 처리할 데이터가 없습니다.")
